<template>
    <div class="relative min-h-screen overflow-hidden bg-white rounded-2xl border border-slate-100 shadow-sm p-8 mb-6">
        <!-- ========================================== -->
        <!-- BACKGROUND: Salud Auditiva + Galáctico     -->
        <!-- ========================================== -->
        <div class="absolute inset-0 z-0 pointer-events-none overflow-hidden">
            <!-- Icono 1: Onda Sonora (Auditiva) - Arriba Izquierda -->
            <svg class="absolute top-[15%] left-[10%] w-24 h-24 text-cyan-600/5 -rotate-12" fill="none"
            ...
            <svg class="absolute top-[20%] right-[15%] w-32 h-32 text-indigo-600/5 animate-[pulse_4s_ease-in-out_infinite]"
                fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <circle cx="12" cy="12" r="5" stroke-width="1" />
                <ellipse cx="12" cy="12" rx="10" ry="3" transform="rotate(30 12 12)" stroke-width="1" />
            </svg>

            <!-- Ícono 3: Oído / Escucha (Auditivo) - Centro Izquierda -->
            <svg class="absolute top-[50%] left-[5%] w-40 h-40 text-slate-600/5" fill="none" stroke="currentColor"
                viewBox="0 0 24 24">
            ...
            <svg class="absolute bottom-[15%] right-[10%] w-48 h-48 text-cyan-700/5" fill="none" stroke="currentColor"
                viewBox="0 0 24 24">
            ...
            <svg class="absolute top-[40%] right-[30%] w-8 h-8 text-slate-500/10 animate-[pulse_3s_ease-in-out_infinite]"
                fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1"
                    d="M12 3v18m9-9H3m12.728-6.364l-12.728 12.728M20.364 20.364L7.636 7.636" />
            </svg>
            <svg class="absolute bottom-[30%] left-[25%] w-12 h-12 text-indigo-500/5" fill="none" stroke="currentColor"
                viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1"
            ...
            <div class="absolute inset-0 bg-gradient-to-b from-transparent via-slate-50/50 to-slate-100/80"></div>
        </div>
        <!-- ========================================== -->
        ...
        <div class="relative z-10">
            <!-- Header -->
            <div class="flex flex-col items-start justify-between mb-8 pb-5 border-b border-slate-100/80">
                <div class="flex items-center gap-4">
                    ...
                    <h1 class="text-sm font-semibold text-slate-400 uppercase tracking-wider">Detalle de la
                        investigación</h1>
                </div>
                <!-- Laoding State -->
                ...
                <div v-else class="space-y-6 mb-8 w-full">
                    <div>
                        <h2 class="text-2xl font-medium  text-slate-800 tracking-tight">{{ research?.name }}</h2>
                    </div>
                    <!-- Contenedor de descripcion minmalista -->
                    <div class="bg-slate-50/50 rounded-xl p-6 border border-slate-100">
                        <p class="text-base text-slate-600 leading-loose  font-light whitespace-pre-wrap">{{
                            research?.description }}</p>
                    </div>
                </div>
            </div>
            <div class="flex flex-wrap items-center justify-between gap-4 pt-6 border-t border-slate-100/80">
                <div class="flex gap-3">
                    ...
                    <!-- Boton de eliminar -->
                    <button class="group flex-1 flex items-center justify-center gap-2 px-4 py-2.5 bg-white border
            border-slate-200 text-slate-500 rounded-xl text-sm font-medium hover:bg-rose-50 hover:border-rose-200
            hover:text-rose-600 transition-colors duration-200">
                ...
                <button @click="openAIModal"
                    ...
                    <svg class="w-4 h-4 text-slate-300 group-hover:text-white transition-colors" fill="none"
                        stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5"
                ...
                <div class="flex items-center justify-between px-6 py-4 border-b border-gray-200 bg-indigo-500">
                    ...
                    <div class="flex items-center gap-2">
                        ...
                        <button @click="fetchAnalysis" :disabled="isLoadingAnalysis"
                            ...
                            <svg v-else class="w-4 h-4 animate-spin" fill="none" stroke="currentColor"
                                viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                    d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" />
                            </svg>
                ...
                <div class="p-6 overflow-y-auto max-h-[calc(90vh-120px)]">
                    ...
                    <div v-else-if="analysisData && analysisData.length > 0" class="space-y-6">
                        ...
                        <div class="flex items-center gap-2 text-sm text-gray-500">
                            ...
                            <span class="ml-auto text-gray-400">{{ analysisData[0]?.notesCount }} notas
                                analizadas</span>
                        </div>
