...
const noteColors = [
  {
    name: "Amarillo",
    value: "bg-yellow-100 border-yellow-300 text-yellow-800",
    color: "#FEF3C7",
  },
  {
    name: "Verde",
    value: "bg-green-100 border-green-300 text-green-800",
    color: "#D1FAE5",
  },
  {
    name: "Azul",
    value: "bg-blue-100 border-blue-300 text-blue-800",
    color: "#DBEAFE",
  },
  {
    name: "Rosa",
    value: "bg-pink-100 border-pink-300 text-pink-800",
    color: "#FCE7F3",
  },
  {
    name: "Púrpura",
    value: "bg-purple-100 border-purple-300 text-purple-800",
    color: "#E9D5FF",
  },
  {
    name: "Naranja",
    value: "bg-orange-100 border-orange-300 text-orange-800",
    color: "#FFEDD5",
  },
];

// ── Delete response ───────────────────────────────────────────────────────────
const deleteResponseDialogOpen = ref(false);
const responsePendingDelete = ref<TinnitusResponse | null>(null);
const deletingResponse = ref(false);

function requestDeleteResponse(response: TinnitusResponse) {
  responsePendingDelete.value = response;
  deleteResponseDialogOpen.value = true;
}

function cancelDeleteResponse() {
  if (deletingResponse.value) return;
  responsePendingDelete.value = null;
  deleteResponseDialogOpen.value = false;
}

async function confirmDeleteResponse() {
  if (!responsePendingDelete.value) return;
  deletingResponse.value = true;
  try {
    await axios.delete(
      `http://localhost:3000/api/v1/tinnitus-responses/${responsePendingDelete.value.id}`,
    );
    responses.value = responses.value.filter(
      (r) => r.id !== responsePendingDelete.value!.id,
    );
  } catch {
    responses.value = responses.value.filter(
      (r) => r.id !== responsePendingDelete.value!.id,
...
const loadResponses = async () => {
  ...
  try {
    const response = await axios.get(
      "http://localhost:3000/api/v1/tinnitus-responses",
      {
        timeout: 30000,
      },
    );
...
const enrichResponseData = async () => {
  ...
  const questionnaireIds = [
    ...new Set(responses.value.map((r) => r.idTinnitusQuestionnaires)),
  ];
  ...
  for (const patientId of patientIds) {
    if (!patients.value.has(patientId)) {
      try {
        const response = await axios.get(
          `http://localhost:3000/api/v1/patients/${patientId}`,
          {
            timeout: 10000,
          },
        );
  ...
  for (const questionnaireId of questionnaireIds) {
    if (!questionnaires.value.has(questionnaireId)) {
      try {
        ...
        console.error(
          `❌ Error al cargar cuestionario ${questionnaireId}:`,
          error,
        );
      }
    }
...
const saveClinicalNote = async () => {
  ...
  try {
    const noteData = {
      ...
      id_tinnitus_questionnaires:
        selectedResponseForNote.value.idTinnitusQuestionnaires,
      id_tinnitus_response: selectedResponseForNote.value.id,
      description: clinicalNoteForm.value.description,
    };

    await axios.post(
      `http://localhost:3000/api/v1/tinnitus-notes/${noteData.id_patient}`,
      noteData,
      {
        timeout: 30000,
        headers: {
...
const loadSavedNotesAnalysis = async (responseId: string) => {
  ...
  try {
    const response = await axios.get(
      `http://localhost:3000/api/v1/tinnitus-notes-analysis/response/${responseId}`,
      {
        timeout: 30000,
      },
    );
...
const saveAIAnalysis = async () => {
  ...
  try {
    const analysisData = {
      ...
      id_tinnitus_questionnaires:
        selectedResponseForAI.value.idTinnitusQuestionnaires,
    ...
    await axios.post(
      "http://localhost:3000/api/v1/tinnitus-analysis",
      analysisData,
      {
        timeout: 30000,
        headers: {
...
const loadSavedAnalysis = async (questionnaireId: string) => {
  ...
  try {
    const response = await axios.get(
      `http://localhost:3000/api/v1/tinnitus-analysis/questionnaire/${questionnaireId}`,
      {
        timeout: 30000,
      },
    );
...
const deleteSavedAnalysis = async () => {
  ...
  try {
    await axios.delete(
      `http://localhost:3000/api/v1/tinnitus-analysis/${savedAnalysis.value.id}`,
      {
        timeout: 30000,
      },
    );
...
const toggleModalView = async () => {
  activeModalView.value =
    activeModalView.value === "analysis" ? "notes" : "analysis";
  // Si cambiamos a notas, cargarlas si no están cargadas aún
  if (
    activeModalView.value === "notes" &&
    analysisNotes.value.length === 0 &&
    !analysisNotesError.value
  ) {
...
const fetchAnalysisNotes = async () => {
  ...
  try {
    ...
    analysisNotes.value = notesData.map((note: any) => ({
      id: note.id,
      title:
        note.source === "analysis-selection"
          ? "Nota destacada del análisis"
...
const analyzeNotesByAI = async () => {
  ...
  try {
    ...
    const notesContent = analysisNotes.value
      .map((note, index) => {
    ...
    notesAnalysisError.value =
      "Error al analizar las notas. Por favor, intente nuevamente.";
...
const saveNotesAnalysis = async () => {
  if (!notesAnalysisResult.value || !selectedResponseForViewAnalysis.value)
    return;
  ...
  try {
    const payload = {
      ...
      id_tinnitus_questionnaires:
        selectedResponseForViewAnalysis.value.idTinnitusQuestionnaires,
    ...
    const response = await axios.post(
      "http://localhost:3000/api/v1/tinnitus-notes-analysis",
      payload,
      {
        timeout: 30000,
...
const saveNote = async (noteId: string | number) => {
  ...
  try {
    await axios.put(
      `http://localhost:3000/api/v1/tinnitus-notes/${noteId}`,
      {
...
const saveContext = async (noteId: string | number) => {
  ...
  try {
    const response = await axios.post(
      `http://localhost:3000/api/v1/tinnitus-notes/${noteId}/context`,
      {
...
const getSeverityLevel = (score: number): { label: string; color: string } => {
  if (score <= 5)
    return { label: "Leve", color: "bg-green-100 text-green-700" };
  if (score <= 10)
    return { label: "Moderado", color: "bg-yellow-100 text-yellow-700" };
  if (score <= 15)
    return { label: "Severo", color: "bg-orange-100 text-orange-700" };
  ...
  <div class="min-h-screen bg-gray-50 relative overflow-hidden">
    ...
    <div class="absolute inset-0 pointer-events-none">
      <div
        ...
        :key="icon.id"
        class="absolute transition-all duration-1000 ease-in-out opacity-10"
        :style="{
          left: `${icon.x}%`,
          top: `${icon.y}%`,
    ...
    <!-- Header -->
    <div
      class="bg-gradient-to-r from-teal-600 to-cyan-600 text-white shadow-lg relative z-10"
    >
        ...
        <div class="flex items-center gap-4">
          ...
          <div>
            <h1 class="text-2xl font-bold">
              Respuestas de Cuestionarios de Tinnitus
            </h1>
            <p class="text-teal-100">
              Visualización de respuestas de pacientes
            </p>
          </div>
        </div>
      </div>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 relative z-10">
      ...
      >
        <div class="relative max-w-md">
          ...
          <input
            v-model="searchQuery"
            type="text"
            placeholder="Buscar por paciente o cuestionario..."
      ...
      <div v-if="isLoading" class="flex items-center justify-center py-12">
        <div
          class="animate-spin rounded-full h-12 w-12 border-b-2 border-teal-600"
        ></div>
      ...
      <div v-else-if="responses.length === 0" class="text-center py-12">
        ...
        <p class="text-gray-500 mt-2">
          No se encontraron respuestas de cuestionarios de tinnitus.
        </p>
      </div>
      ...
      <div v-else class="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
        ...
        >
          ...
          <div class="flex items-start justify-between mb-4">
            ...
            <span
              :class="
                getSeverityLevel(calculateTotalScore(response.answer)).color
              "
          ...
          <div class="border-l-2 border-gray-100 ml-5 pl-3.5 mb-4">
            ...
            <div class="flex flex-wrap gap-1.5">
              <span
                v-for="(ans, idx) in response.answer?.slice(0, 2)"
                :key="idx"
              ...
              <span
                v-if="(response.answer?.length || 0) > 2"
          ...
          <div class="border-t border-gray-100 pt-4 grid grid-cols-2 gap-2">
            ...
            <!-- Nota Clínica -->
            <button
              @click.stop="openClinicalNoteModal(response)"
            ...
            <button
              @click.stop="openAIAnalysisModal(response)"
            ...
            <!-- Ver Notas -->
            <button
              @click.stop="openViewAnalysisModal(response)"
            ...
            <button
              @click.stop="openViewNotesModal(response)"
            ...
            <!-- Eliminar Respuesta -->
            <button
              @click.stop="requestDeleteResponse(response)"
              class="col-span-2 flex items-center justify-center gap-1.5 py-2 text-rose-600 bg-white rounded-lg text-xs font-medium ring-1 ring-rose-200/70 hover:bg-rose-50/70 hover:ring-rose-300/70 transition-all"
            >
              <svg
                class="w-3.5 h-3.5 shrink-0"
                fill="none"
    ...
    <Teleport to="body">
      <Transition name="app-modal">
        <div
          v-if="deleteResponseDialogOpen && responsePendingDelete"
        ...
        >
          <div
            ...
            aria-hidden="true"
            @click="cancelDeleteResponse"
          />
          <div
            class="app-modal-scrim app-modal-scrim--sheet"
            @click.self="cancelDeleteResponse"
          >
            <div
              role="alertdialog"
              aria-modal="true"
              aria-labelledby="del-resp-title"
              aria-describedby="del-resp-desc"
              class="pointer-events-auto w-full max-w-md overflow-hidden rounded-2xl bg-white shadow-2xl ring-1 ring-slate-200/80"
              @click.stop
            >
              <!-- Header -->
              <header
                class="border-b border-teal-200/80 bg-gradient-to-r from-teal-50 via-white to-cyan-50 px-6 py-4"
              >
                <div class="flex items-start gap-3">
                  <span
                    class="inline-flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-teal-100 text-teal-700 ring-1 ring-teal-200/80"
                    aria-hidden="true"
                  >
                    <svg
                      class="h-5 w-5"
                  ...
                  <div class="min-w-0">
                    ...
                    >
                      ¿Eliminar esta respuesta clínica?
                    </h2>
                    <p class="mt-1 text-sm font-medium text-slate-500 truncate">
                      {{ responsePendingDelete.patientName }} ·
                      {{ responsePendingDelete.questionnaireTitle }}
                    </p>
                  </div>
                </div>
              </header>
              ...
              <div class="px-6 py-5">
                ...
                >
                  Las respuestas de los pacientes son la
                  <strong class="font-semibold text-slate-900"
                    >materia prima del diagnóstico clínico</strong
                  >. Cada valor registrado aquí permite detectar patrones de
                  severidad, anticipar
                  <strong class="font-semibold text-teal-800"
                    >posibles enfermedades auditivas progresivas</strong
                  >
                  y orientar el plan de tratamiento personalizado del paciente.
                </p>
                <p class="mt-3 text-sm leading-relaxed text-slate-700">
                  Eliminar esta respuesta borrará de forma permanente sus
                  puntuaciones, el historial de análisis de IA vinculados y las
                  notas clínicas asociadas — datos que
                  <strong class="font-semibold text-rose-700"
                    >no podrán recuperarse</strong
                  >
                  y que podrían ser determinantes para un diagnóstico oportuno o
              ...
              <!-- Footer -->
              <footer
                class="flex flex-col gap-2 border-t border-slate-200 bg-slate-50/80 px-6 py-4 sm:flex-row sm:justify-end"
              >
                <button
                  type="button"
                  class="inline-flex items-center justify-center rounded-xl px-4 py-2.5 text-sm font-semibold text-slate-700 ring-1 ring-slate-200/80 transition-all hover:bg-white disabled:opacity-50"
                ...
                <button
                  ...
                  :disabled="deletingResponse"
                  @click="confirmDeleteResponse"
                >
                  {{
                    deletingResponse ? "Eliminando…" : "Eliminar de todos modos"
                  }}
                </button>
              </footer>
            </div>
    ...
    <Teleport to="body">
      <Transition name="app-modal">
        ...
        >
          ...
          >
            <div
              class="app-modal-panel app-modal-panel--sheet bg-white rounded-2xl shadow-2xl w-full max-w-3xl max-h-[90vh] overflow-hidden"
              role="dialog"
              aria-modal="true"
              @click.stop
            >
              ...
              <div class="p-6 overflow-y-auto max-h-[calc(90vh-200px)]">
                ...
                <div class="grid grid-cols-2 gap-4 mb-6">
                  ...
                  <div class="bg-gray-50 rounded-lg p-4">
                    ...
                    <p class="font-semibold text-gray-900">
                      {{ selectedResponse.questionnaireTitle }}
                    </p>
                  </div>
                </div>
                ...
                <!-- Total Score -->
                <div
                  class="bg-teal-50 border border-teal-200 rounded-xl p-4 mb-6"
                >
                  <div class="flex items-center justify-between">
                    <div>
                ...
                <div class="space-y-4">
                  ...
                  <div
                    v-for="(answer, idx) in selectedResponse.answer"
                    :key="idx"
              ...
              <div class="bg-gray-50 px-6 py-4 flex justify-between">
                <button
                  @click="openClinicalNoteModal(selectedResponse)"
    ...
    <Teleport to="body">
      <Transition name="app-modal">
        <div
          v-if="showClinicalNoteModal && selectedResponseForNote"
          class="app-modal-root"
          style="z-index: 60"
        >
          <div
            class="app-modal-backdrop"
            aria-hidden="true"
            @click="closeClinicalNoteModal"
          />
          <div
            class="app-modal-scrim app-modal-scrim--sheet"
            @click.self="closeClinicalNoteModal"
          >
            <div
              class="app-modal-panel app-modal-panel--sheet bg-white rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-hidden"
              role="dialog"
              aria-modal="true"
              @click.stop
            >
                  ...
                  <div class="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p class="text-blue-100 text-sm">
                        {{ selectedResponseForNote.questionnaireTitle }}
                      </p>
                    </div>
                  </div>
                  <button
              ...
              <div class="p-6 overflow-y-auto max-h-[calc(90vh-200px)]">
                ...
                <div class="bg-gray-50 rounded-lg p-4 mb-6">
                  ...
                  <p class="font-semibold text-gray-900">
                    {{ selectedResponseForNote.patientName }}
                  </p>
                </div>
                ...
                <div class="space-y-2">
                  <label
                    for="clinicalNoteDescription"
                    class="block text-sm font-medium text-gray-700"
                  >
                    Descripción de la Nota Clínica
                    <span class="text-red-500">*</span>
                  </label>
                  <textarea
                    id="clinicalNoteDescription"
                    v-model="clinicalNoteForm.description"
                    rows="6"
              ...
              <div class="bg-gray-50 px-6 py-4 flex justify-end gap-3">
                ...
                <button
                  ...
                  :disabled="
                    isSavingNote || !clinicalNoteForm.description.trim()
                  "
    ...
    <Teleport to="body">
      <Transition name="app-modal">
        <div
          v-if="showViewNotesModal && selectedResponseForView"
          class="app-modal-root"
          style="z-index: 60"
        >
          <div
            class="app-modal-backdrop"
            aria-hidden="true"
            @click="closeViewNotesModal"
          />
          <div
            class="app-modal-scrim app-modal-scrim--sheet"
            @click.self="closeViewNotesModal"
          >
            <!-- Modal Content -->
            <div
              role="dialog"
              aria-modal="true"
              :class="[
                'relative bg-white rounded-2xl shadow-2xl overflow-hidden transition-all duration-300 pointer-events-auto',
                isClinicalNotesModalExpanded
            ...
            >
                ...
                <div class="flex items-center justify-between">
                  <div class="flex items-center gap-3">
                    ...
                    <div>
                      <h2 class="text-xl font-bold text-white">
                        {{
                          clinicalNotesModalView === "notes"
                            ? "Notas Clínicas"
                            : "Análisis de Notas"
                        }}
                      </h2>
                      <p class="text-purple-100 text-sm">
                        {{ selectedResponseForView.questionnaireTitle }}
                      </p>
                    </div>
                  </div>
                  <div class="flex items-center gap-2">
                    ...
                    <button
                      ...
                      :disabled="isLoadingNotesAnalysis"
                    ...
                    >
                      <svg
                        v-if="clinicalNotesModalView === 'notes'"
                        class="w-4 h-4"
                        fill="none"
                        stroke="currentColor"
                        viewBox="0 0 24 24"
                    ...
                    <button
                      @click="toggleExpandClinicalNotesModal"
                    ...
                    >
                      <svg
                        v-if="!isClinicalNotesModalExpanded"
                        class="w-4 h-4"
                        fill="none"
                        stroke="currentColor"
                        viewBox="0 0 24 24"
                      ...
                      <span>{{
                        isClinicalNotesModalExpanded ? "Contraer" : "Expandir"
                      }}</span>
                    </button>
              ...
              >
                ...
                <div class="bg-gray-50 rounded-lg p-4 mb-4">
                  ...
                  <p class="font-semibold text-gray-900">
                    {{ selectedResponseForView.patientName }}
                  </p>
                </div>
                ...
                <template v-if="clinicalNotesModalView === 'notes'">
                  ...
                  <!-- Empty State -->
                  <div
                    v-else-if="clinicalNotes.length === 0"
                  ...
                  <div v-else class="space-y-4">
                    ...
                    >
                      ...
                      <div
                        class="mt-3 pt-3 border-t border-purple-200 flex items-center justify-between"
                      >
                ...
                <template v-else>
                  <!-- Loading State -->
                  <div
                    v-if="isLoadingNotesAnalysis"
                    class="flex flex-col items-center justify-center py-12"
                  >
                    <div
                      class="animate-spin rounded-full h-12 w-12 border-b-2 border-violet-600 mb-4"
                    ></div>
                  ...
                  <!-- Error State -->
                  <div
                    v-else-if="!savedNotesAnalysis && !isLoadingNotesAnalysis"
                    class="text-center py-12"
                  >
                    <div
                      class="w-12 h-12 bg-violet-100 rounded-full flex items-center justify-center mx-auto mb-3"
                    >
                      <svg
                    ...
                    <h3 class="text-lg font-medium text-gray-900">
                      No hay análisis guardado
                    </h3>
                    <p class="text-gray-500 mt-2">
                      Aún no se ha generado un análisis de IA para estas notas.
                    </p>
                    <button
                      @click="
                        clinicalNotesModalView = 'notes';
                        openAIAnalysisModal(selectedResponseForView);
                      "
                  ...
                  <div v-else class="bg-white rounded-lg">
                    <div
                      class="prose prose-purple max-w-none"
                      v-html="renderMarkdown(savedNotesAnalysis || '')"
                    ></div>
                  </div>
                </template>
              </div>
    ...
    <Teleport to="body">
      <Transition name="app-modal">
        <div
          v-if="showAIAnalysisModal && selectedResponseForAI"
          class="app-modal-root"
          style="z-index: 60"
        >
          <div
          ...
          >
            <div
              class="app-modal-panel app-modal-panel--sheet bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-hidden"
              role="dialog"
              aria-modal="true"
              @click.stop
            >
              <!-- Header -->
              <div
                class="bg-gradient-to-r from-violet-600 to-purple-600 px-6 py-4"
              >
                <div class="flex items-center justify-between">
                  <div class="flex items-center gap-3">
                    <div
                    ...
                    <div>
                      ...
                      <p class="text-violet-100 text-sm">
                        {{ selectedResponseForAI.questionnaireTitle }}
                      </p>
                    </div>
                  </div>
                  <button
              ...
              <div class="p-6 overflow-y-auto max-h-[calc(90vh-200px)]">
                ...
                <div class="bg-gray-50 rounded-lg p-4 mb-4">
                  <div class="flex items-center justify-between">
                    <div>
                      <p
                        class="text-xs text-gray-500 uppercase font-medium mb-1"
                      >
                        Paciente
                      </p>
                      <p class="font-semibold text-gray-900">
                        {{ selectedResponseForAI.patientName }}
                      </p>
                    </div>
                    <div class="text-right">
                      ...
                      <span
                        :class="
                          getSeverityLevel(
                            calculateTotalScore(selectedResponseForAI.answer),
                          ).color
                        "
                ...
                <div v-else class="space-y-4">
                  <div
                    class="bg-violet-50 rounded-lg p-4 border border-violet-200"
                  >
                    <h4
                      class="text-sm font-semibold text-violet-800 mb-3 flex items-center gap-2"
                    >
                      <svg
                        class="w-4 h-4"
                        fill="none"
              ...
              <div class="bg-gray-50 px-6 py-4 flex justify-between">
                ...
                <button
                  ...
                  :disabled="
                    isAnalyzing || !aiAnalysisResult || isSavingAnalysis
                  "
                ...
                >
                  ...
                  <span>{{
                    isSavingAnalysis ? "Guardando..." : "Guardar Análisis"
                  }}</span>
                </button>
              </div>
            </div>
    ...
    <Teleport to="body">
      <Transition name="app-modal">
        <div
          v-if="showViewAnalysisModal && selectedResponseForViewAnalysis"
          class="app-modal-root"
          style="z-index: 60"
        >
          <div
            class="app-modal-backdrop"
            aria-hidden="true"
            @click="closeViewAnalysisModal"
          />
          <div
            class="app-modal-scrim app-modal-scrim--sheet"
            @click.self="closeViewAnalysisModal"
          >
            <!-- Modal Content -->
            <div
              role="dialog"
              aria-modal="true"
              :class="[
                'relative bg-white rounded-2xl shadow-2xl w-full overflow-hidden transition-all duration-300 pointer-events-auto',
                isExpanded
            ...
            >
              ...
              >
                <div class="flex items-center justify-between">
                  <div class="flex items-center gap-3">
                    ...
                    <div>
                      <h2 class="text-xl font-bold text-white">
                        Análisis de la respuesta del cuestionario
                      </h2>
                      <p class="text-amber-100 text-sm">
                        {{ selectedResponseForViewAnalysis.questionnaireTitle }}
                      </p>
                    </div>
                  </div>
                  <div class="flex items-center gap-2">
                    ...
                    <button
                      ...
                      :disabled="isLoadingAnalysisNotes"
                      class="inline-flex items-center gap-2 px-3 py-1.5 text-sm font-medium rounded-lg border border-white/40 bg-white/10 text-white hover:bg-white/20 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                    >
                      <!-- Icono: libreta para notas, gráfico para análisis -->
                      <svg
                        v-if="activeModalView === 'analysis'"
                        class="w-4 h-4"
                        fill="none"
                        stroke="currentColor"
                        viewBox="0 0 24 24"
              ...
              >
                ...
                <template v-if="activeModalView === 'analysis'">
                  ...
                  <div v-else-if="!savedAnalysis" class="text-center py-12">
                    ...
                    <p class="text-gray-500 mt-2">
                      Este cuestionario aún no tiene un análisis de IA guardado.
                    </p>
                    <button
                      @click="
                        closeViewAnalysisModal;
                        openAIAnalysisModal(selectedResponseForViewAnalysis);
                      "
                      class="mt-4 px-6 py-2 bg-gradient-to-r from-violet-600 to-purple-600 text-white rounded-lg font-medium hover:from-violet-700 hover:to-purple-700 transition-all"
                    >
                      Crear Análisis con IA
                  ...
                  <div v-else class="space-y-4">
                    ...
                    <div class="bg-gray-50 rounded-lg p-4">
                      <div class="flex items-center justify-between">
                        <div>
                          ...
                          <p class="font-semibold text-gray-900">
                            {{ selectedResponseForViewAnalysis.patientName }}
                          </p>
                        </div>
                        <div class="text-right">
                          <p
                    ...
                    >
                      <div class="flex items-center justify-between mb-3">
                        <h4
                          class="text-sm font-semibold text-amber-800 flex items-center gap-2"
                        >
                          <svg
                            class="w-4 h-4"
                            fill="none"
                      ...
                      <div
                        ...
                        v-html="
                          savedAnalysis.analysis
                            ? md.render(savedAnalysis.analysis)
                            : '<p>Sin análisis disponible</p>'
                        "
                      ></div>
                      ...
                      <div
                        v-if="selectedText"
                        class="mt-4 p-3 bg-white rounded border border-amber-300"
                      >
                ...
                <template v-else-if="activeModalView === 'notes'">
                  ...
                  <div
                    v-if="isLoadingAnalysisNotes"
                    class="flex flex-col items-center justify-center py-12 text-gray-400"
                  >
                    <svg
                      class="animate-spin w-8 h-8 mb-3"
                      fill="none"
                      viewBox="0 0 24 24"
                    >
                      <circle
                  ...
                  <!-- Estado 3: Sin notas -->
                  <div
                    v-else-if="analysisNotes.length === 0"
                    class="flex flex-col items-center justify-center py-12 text-gray-400"
                  >
                    <svg
                  ...
                  <ul
                    v-else
                    class="divide-y divide-gray-100 space-y-0 max-h-96 overflow-y-auto"
                  >
                    <li
                      v-for="note in analysisNotes"
                      :key="note.id"
                      class="py-4"
                      :class="note.color || 'bg-gray-50'"
                    >
                      <div
                      ...
                      >
                        <div class="flex-1 min-w-0">
                          ...
                          <div class="flex items-center gap-2 mb-2">
                            <div
                              v-if="note.color"
                              class="w-3 h-3 rounded-md shadow-sm"
                              :class="
                                note.color.split(' ')[0].replace('bg-', 'bg-')
                              "
                              :style="{
                                backgroundColor:
                                  noteColors.find((c) => c.value === note.color)
                                    ?.color || '#FEF3C7',
                              }"
                            ></div>
                            <p
                          ...
                          <div v-else>
                            ...
                            <div
                              v-if="note.context && note.context.length > 0"
                              class="mt-3 ml-2"
                            >
                              <div class="flex items-center gap-1 mb-2">
                                <div class="w-4 h-px bg-gray-300"></div>
                                <span class="text-xs text-gray-500 font-medium"
                                  >Contextos ({{ note.context.length }})</span
                                >
                              </div>
                              <div
                                class="relative pl-3 border-l-2 border-gray-200 space-y-2"
                              >
                                <div
                                  v-for="(ctx, ctxIndex) in note.context"
                                  :key="ctx.id"
                                  class="relative p-2 rounded-lg transition-all duration-200 hover:bg-white/50"
                                  :class="[
                                    ctxIndex % 2 === 0
                                      ? 'bg-white/30 ml-0'
                                      : 'bg-white/10 ml-3',
                                ...
                                >
                                  ...
                                  <div class="flex items-start gap-2">
                                    ...
                                    <div class="flex-1 min-w-0">
                                      <p
                                        class="text-xs text-gray-600 whitespace-pre-line leading-relaxed"
                                        :class="
                                          ctxIndex % 2 === 0
                                            ? 'text-left'
                                            : 'text-right'
                                      ...
                                      <span
                                        class="text-[10px] text-gray-400 block mt-1"
                                        :class="
                                          ctxIndex % 2 === 0
                                            ? 'text-left'
                                            : 'text-right'
                            ...
                            >
                              ...
                              <textarea
                                ...
                                placeholder="Escribe el contexto adicional..."
                              ></textarea>
                              <div class="flex gap-2 mt-2 justify-end">
                                <button
                                  @click="cancelAddingContext"
                        ...
                        >
                          ...
                          <div class="flex items-center gap-1">
                            ...
                            <button
                              @click="startAddingContext(note.id)"
                              :disabled="
                                addingContextToNoteId !== null ||
                                editingNoteId !== null
                              "
              ...
              >
                <div class="flex items-center gap-3">
                  ...
                  >
                    <svg
                      v-if="isCreatingNoteFromSelection"
                      class="animate-spin h-4 w-4"
                      fill="none"
                      viewBox="0 0 24 24"
                    >
                    ...
                    <span>{{
                      isCreatingNoteFromSelection ? "Creando..." : "Crear Nota"
                    }}</span>
                  </button>
                  <!-- Analyze Notes Button -->
                  <button
                    v-if="
                      activeModalView === 'notes' && analysisNotes.length > 0
                    "
                    @click="analyzeNotesByAI"
                ...
                >
                  ...
                  <span>{{
                    isDeletingAnalysis ? "Eliminando..." : "Eliminar"
                  }}</span>
                </button>
              </div>
              ...
              <div
                ...
                :style="{
                  ...
                  top: colorMenuPosition.y - 120 + 'px',
                }"
                @click.stop
              >
                <p class="text-xs text-gray-500 font-medium mb-2 px-2">
                  Seleccione color para la nota:
                </p>
                <div class="space-y-1">
                  <button
                    v-for="color in noteColors"
                    :key="color.name"
                    @click="createNoteFromSelection(color.value)"
        ...
        >
          ...
          >
            ...
            >
              <!-- Header -->
              <div
                class="bg-gradient-to-r from-purple-600 to-pink-600 px-6 py-4 flex justify-between items-center"
              >
                <div class="flex items-center gap-3">
                  <div
                ...
                <div class="flex items-center gap-2">
                  ...
                  <button
                    @click="toggleNotesAnalysisModalSize"
                    class="text-white/80 hover:text-white transition-colors p-1"
                    :title="
                      isNotesAnalysisModalExpanded ? 'Contraer' : 'Expandir'
                    "
                  >
                    <svg
                      v-if="isNotesAnalysisModalExpanded"
                      class="w-5 h-5"
                      fill="none"
                      stroke="currentColor"
                      viewBox="0 0 24 24"
              ...
              <div class="flex-1 overflow-y-auto p-6">
                ...
                >
                  <svg
                    class="animate-spin h-10 w-10 text-purple-600 mb-4"
                    fill="none"
                    viewBox="0 0 24 24"
                  >
                    <circle
                ...
                <div v-else-if="notesAnalysisResult" class="space-y-6">
                  ...
                  >
                    ...
                    <p class="text-sm text-purple-600">
                      Se analizaron {{ analysisNotes.length }} notas. Este
                      análisis fue generado por IA y debe ser revisado por un
                      profesional médico.
                    </p>
                  </div>
                  ...
                  <div class="prose prose-purple max-w-none">
                    <div
                      v-html="md.render(notesAnalysisResult)"
                      class="markdown-content text-gray-700 leading-relaxed"
                    ></div>
                  </div>
                </div>
              ...
              >
                ...
                <button
                  v-if="notesAnalysisResult && !savedNotesAnalysisId"
                  @click="saveNotesAnalysis"
                  :disabled="isSavingNotesAnalysis"
                  class="px-6 py-2 bg-gradient-to-r from-purple-600 to-pink-600 text-white rounded-lg font-medium hover:from-purple-700 hover:to-pink-700 transition-all flex items-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  <svg
                    v-if="isSavingNotesAnalysis"
                    class="animate-spin h-4 w-4"
                    fill="none"
                    viewBox="0 0 24 24"
                  >
                  ...
                  <span>{{
                    isSavingNotesAnalysis ? "Guardando..." : "Guardar Análisis"
                  }}</span>
                </button>
                <div
                  v-if="savedNotesAnalysisId"
