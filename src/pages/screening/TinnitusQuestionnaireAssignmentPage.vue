...
const activeCount = computed(
  () =>
    questionnaires.value.filter((q) => q.status === QuestionnaireStatus.Active)
      .length,
);
const inactiveCount = computed(
  () =>
    questionnaires.value.filter(
      (q) => q.status === QuestionnaireStatus.Inactive,
    ).length,
);
const discontinuedCount = computed(
  () =>
    questionnaires.value.filter(
      (q) => q.status === QuestionnaireStatus.Discontinued,
    ).length,
);
...
const hasSelectedQuestionnaires = computed(
  () => selectedQuestionnaires.value.size > 0,
);
...
const selectedCountText = computed(() => {
  ...
  return count === 1
    ...
    : `${count} cuestionarios seleccionados`;
});
...
const loadQuestionnaires = async () => {
  ...
  try {
    ...
    showAlert(
      "Error al cargar los cuestionarios. Intente nuevamente.",
      "error",
    );
  } finally {
...
const selectAllQuestionnaires = () => {
  if (
    selectedQuestionnaires.value.size === filteredQuestionnaires.value.length
  ) {
    selectedQuestionnaires.value.clear();
  } else {
    filteredQuestionnaires.value.forEach((q) =>
      selectedQuestionnaires.value.add(q.id),
    );
  }
};
...
const assignQuestionnaires = async () => {
  ...
  try {
    ...
    console.log("📤 Asignando cuestionarios:", {
      questionnaireIds,
      patientIds,
    });
    ...
    showAlert(
      error.response?.data?.message ||
        "Error al asignar los cuestionarios. Intente nuevamente.",
      "error",
    );
  } finally {
...
const updateQuestionnaireStatus = async (
  ...
  try {
    ...
    showAlert(
      `Cuestionario marcado como "${statusLabels[newStatus]}"`,
      "success",
    );
...
const toggleStatusMenu = (questionnaireId: string) => {
  statusMenuOpen.value =
    statusMenuOpen.value === questionnaireId ? null : questionnaireId;
};
...
<template>
  <div
    class="min-h-screen max-w-7xl mx-auto bg-gray-50 relative overflow-hidden"
  >
    <!-- Background con iconos de salud auditiva - Sutiles y pequeños -->
    <div class="absolute inset-0 pointer-events-none" aria-hidden="true">
      <!-- Nota Musical -->
      <svg
        class="absolute top-16 left-8 w-7 h-7 text-slate-200 opacity-40"
        viewBox="0 0 24 24"
        fill="currentColor"
      >
        <path
      ...
      <!-- Corchetes -->
      <svg
        class="absolute top-24 right-16 w-7 h-7 text-emerald-200 opacity-35"
        viewBox="0 0 24 24"
        fill="currentColor"
      >
      ...
      <!-- Auriculares -->
      <svg
        class="absolute top-40 left-20 w-7 h-7 text-rose-300 opacity-40"
        viewBox="0 0 24 24"
        fill="currentColor"
      >
        <path
      ...
      <!-- Parlantes -->
      <svg
        class="absolute top-32 right-32 w-7 h-7 text-slate-300 opacity-35"
        viewBox="0 0 24 24"
        fill="currentColor"
      >
        <path
      ...
      <!-- Clave de sol -->
      <svg
        class="absolute bottom-40 left-12 w-7 h-7 text-amber-300 opacity-40"
        viewBox="0 0 24 24"
        fill="currentColor"
      >
        <path
      ...
      <!-- Ecualizador -->
      <svg
        class="absolute top-48 right-24 w-7 h-7 text-sky-300 opacity-35"
        viewBox="0 0 24 24"
        fill="currentColor"
      >
        <path
      ...
      <!-- Disco de vinilo -->
      <svg
        class="absolute bottom-32 right-20 w-7 h-7 text-rose-400 opacity-30"
        viewBox="0 0 24 24"
        fill="currentColor"
      >
      ...
      <!-- Microfono -->
      <svg
        class="absolute top-20 left-1/3 w-7 h-7 text-emerald-300 opacity-35"
        viewBox="0 0 24 24"
        fill="currentColor"
      >
        <path
      ...
      <!-- Pentagrama -->
      <svg
        class="absolute bottom-20 left-1/4 w-7 h-7 text-slate-400 opacity-30"
        viewBox="0 0 24 24"
        fill="currentColor"
      >
      ...
      <!-- Pulso musical -->
      <svg
        class="absolute top-60 left-16 w-7 h-7 text-amber-400 opacity-35"
        viewBox="0 0 24 24"
        fill="currentColor"
      >
        <path
          d="M3 12h2v4H3v-4zm4-3h2v15H7V9zm4-4h2v19h-2V5zm4 6h2v13h-2V11zm4-5h2v18h-2V6z"
        />
      </svg>

      <!-- Oido -->
      <svg
        class="absolute top-36 left-1/2 w-7 h-7 text-slate-200 opacity-40"
        viewBox="0 0 24 24"
        fill="currentColor"
      >
        <path
      ...
      <!-- Percepcion -->
      <svg
        class="absolute bottom-48 right-12 w-7 h-7 text-emerald-400 opacity-30"
        viewBox="0 0 24 24"
        fill="currentColor"
      >
        <path
      ...
      <!-- Audicion Sana -->
      <svg
        class="absolute top-16 right-8 w-7 h-7 text-rose-200 opacity-35"
        viewBox="0 0 24 24"
        fill="currentColor"
      >
        <path
      ...
      <!-- Protecion -->
      <svg
        class="absolute bottom-16 left-8 w-7 h-7 text-amber-200 opacity-35"
        viewBox="0 0 24 24"
        fill="currentColor"
      >
        <path
      ...
      <!-- Audiograma -->
      <svg
        class="absolute top-1/2 left-8 w-7 h-7 text-sky-400 opacity-30"
        viewBox="0 0 24 24"
        fill="currentColor"
      >
        <path
      ...
      <!-- Alerta auditiva -->
      <svg
        class="absolute bottom-24 right-8 w-7 h-7 text-rose-300 opacity-35"
        viewBox="0 0 24 24"
        fill="currentColor"
      >
        <path
      ...
      <!-- Audifono -->
      <svg
        class="absolute top-44 left-1/3 w-7 h-7 text-emerald-200 opacity-35"
        viewBox="0 0 24 24"
        fill="currentColor"
      >
        <path
      ...
      <!-- Volumen seguro -->
      <svg
        class="absolute bottom-36 left-1/3 w-7 h-7 text-slate-300 opacity-30"
        viewBox="0 0 24 24"
        fill="currentColor"
      >
        <path
      ...
      <!-- Test Auditivo -->
      <svg
        class="absolute top-28 right-1/4 w-7 h-7 text-amber-300 opacity-35"
        viewBox="0 0 24 24"
        fill="currentColor"
      >
        <path
      ...
      <!-- Sin ruido -->
      <svg
        class="absolute bottom-12 right-1/3 w-7 h-7 text-sky-200 opacity-35"
        viewBox="0 0 24 24"
        fill="currentColor"
      >
        <path
      ...
      <!-- Bienestar -->
      <svg
        class="absolute top-52 right-16 w-7 h-7 text-emerald-300 opacity-30"
        viewBox="0 0 24 24"
        fill="currentColor"
      >
        <path
      ...
      <!-- Frecuencia -->
      <svg
        class="absolute bottom-8 left-16 w-7 h-7 text-rose-400 opacity-30"
        viewBox="0 0 24 24"
        fill="currentColor"
      >
        <path
          ...
          <div class="flex items-center gap-4">
            ...
            <div>
              ...
              <p class="text-teal-100 text-sm">
                Seleccione cuestionarios y asígnelos a pacientes
              </p>
            </div>
          </div>
          <div
            class="hidden sm:flex items-center gap-2 text-sm text-teal-100"
    ...
    <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
      ...
      >
        ...
        >
          ...
          >
            ...
            <button
              @click="setStatusFilter(QuestionnaireStatus.Active)"
              :class="[
            ...
            <button
              @click="setStatusFilter(QuestionnaireStatus.Inactive)"
              :class="[
                'px-4 py-2 rounded-lg text-sm font-medium transition-all flex items-center gap-1.5',
            ...
            <button
              @click="setStatusFilter(QuestionnaireStatus.Discontinued)"
              :class="[
                'px-4 py-2 rounded-lg text-sm font-medium transition-all flex items-center gap-1.5',
          ...
          <div class="relative w-full lg:w-80">
            <label for="search-questionnaires" class="sr-only"
      ...
      <div class="sticky top-0 z-30 bg-gray-50 py-3 -mt-3">
        <div
          class="bg-white rounded-xl shadow-md border border-gray-200 px-4 py-3 flex items-center justify-between"
        >
          <div class="flex items-center gap-3">
            <input
              ...
              id="select-all"
              :checked="
                selectedQuestionnaires.size === filteredQuestionnaires.length &&
            ...
            >
              Seleccionar visible{{
                filteredQuestionnaires.length > 0
                  ? `s (${filteredQuestionnaires.length})`
                  : ""
              }}
            </label>
            <span
              v-if="hasSelectedQuestionnaires"
          ...
          <button
            ...
            :disabled="!hasSelectedQuestionnaires"
            ...
            :aria-describedby="
              !hasSelectedQuestionnaires ? 'select-hint' : undefined
            "
          >
            <svg
              class="w-5 h-5"
        ...
        <p v-if="!hasSelectedQuestionnaires" id="select-hint" class="sr-only">
          Seleccione al menos un cuestionario para habilitar la asignación
        </p>
      </div>

      <!-- Loading State -->
      <div
        v-if="isLoadingQuestionnaires"
      ...
      >
        ...
        <p class="text-gray-500 mt-2">
          No se encontraron cuestionarios de tinnitus en el sistema.
        </p>
      </div>

      <!-- No Results after Filter -->
      <div
        v-else-if="filteredQuestionnaires.length === 0"
        class="text-center py-16"
        role="status"
      >
        <div
        ...
        <button
          @click="setStatusFilter('all')"
          class="mt-4 px-4 py-2 text-teal-600 hover:text-teal-700 font-medium"
        >
      ...
      <div
        ...
        class="space-y-3"
        role="list"
        aria-label="Lista de cuestionarios"
      >
        <article
          v-for="questionnaire in filteredQuestionnaires"
          :key="questionnaire.id"
          role="listitem"
          class="bg-white rounded-xl border-2 p-4 transition-all cursor-pointer"
          :class="[
          ...
          @click="
            isQuestionnaireSelectable(questionnaire.status) &&
          ...
          "
          tabindex="0"
          :aria-selected="selectedQuestionnaires.has(questionnaire.id)"
          :aria-disabled="!isQuestionnaireSelectable(questionnaire.status)"
        >
          <div class="flex items-start gap-4">
            <input
              type="checkbox"
              :checked="selectedQuestionnaires.has(questionnaire.id)"
              @click.stop
            ...
            <div class="flex-1 min-w-0">
              <div class="flex items-start justify-between gap-3">
                ...
                <div class="flex items-center gap-2 shrink-0">
                  <span
                    class="px-2.5 py-1 bg-gray-100 text-gray-600 rounded-lg text-xs font-medium uppercase"
                  >
                    {{ questionnaire.type }}
                  </span>
                  ...
                  <div class="relative">
                    <button
                      @click.stop="toggleStatusMenu(questionnaire.id)"
                      class="p-1.5 rounded-lg hover:bg-gray-100 transition-colors"
                      :aria-label="`Cambiar estado de ${questionnaire.title}`"
                      aria-haspopup="true"
                    ...
                    <div
                      v-if="statusMenuOpen === questionnaire.id"
                      class="absolute right-0 mt-1 w-48 bg-white rounded-lg shadow-lg border border-gray-200 py-1 z-20"
                      role="menu"
                    >
                      <button
                        v-for="status in Object.values(QuestionnaireStatus)"
                        :key="status"
                        @click.stop="
                          updateQuestionnaireStatus(questionnaire.id, status)
                        "
                      ...
                      >
                        <span
                          ...
                          :class="{
                            ...
                            'bg-gray-400':
                              status === QuestionnaireStatus.Inactive,
                            'bg-red-500':
                              status === QuestionnaireStatus.Discontinued,
                          }"
                        ></span>
                        {{ statusLabels[status] }}
                      </button>
              ...
              <div
                class="flex flex-wrap items-center gap-3 mt-3 text-sm text-gray-500"
              >
            ...
            >
                ...
                <div class="flex items-center justify-between">
                  <div class="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p class="text-teal-100 text-sm">
                        Seleccione los pacientes para asignar los cuestionarios
                      </p>
                    </div>
                  </div>
                  <button
              ...
              <div class="p-6 overflow-y-auto max-h-[calc(90vh-200px)]">
                ...
                <div class="relative mb-4">
                  <label for="patient-search" class="sr-only"
                    >Buscar pacientes</label
                  >
                  <div
                    class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none"
                  >
                ...
                >
                  <div
                    ...
                    @keydown.enter="togglePatient(patient.id)"
                    tabindex="0"
                  >
                    <input
                      type="checkbox"
                ...
                <div
                  ...
                  class="mt-4 p-3 bg-teal-50 border border-teal-200 rounded-lg"
                >
              ...
              <div class="bg-gray-50 px-6 py-4 flex justify-end gap-3">
                ...
                <button
                  @click="assignQuestionnaires"
                  :disabled="!hasSelectedPatients || isAssigning"
