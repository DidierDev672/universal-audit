<template>
  <div class="max-w-5xl mx-auto">
    <!-- Header -->
    <div
      class="bg-gradient-to-r from-blue-600 via-indigo-600 to-purple-600 rounded-3xl p-8 text-white shadow-2xl mb-8"
    >
      <div class="flex items-center gap-4">
        ...
        <div>
          ...
          <p class="text-white/90">
            Crea evaluaciones de identificación de sonidos
          </p>
        </div>
      </div>
    </div>
    ...
    >
      <!-- Progress Indicator -->
      <div
        class="bg-gradient-to-r from-blue-50 to-indigo-50 px-8 py-4 border-b border-blue-100"
      >
      ...
      <form @submit.prevent="handleSubmit" class="p-8 space-y-8">
        ...
        <div class="space-y-2">
          ...
          <p class="text-xs text-gray-500 flex items-start gap-2">
            ...
            <span
              >Categoría del tipo de sonido que se evaluará (frecuencias, tonos,
              palabras, etc.)</span
            >
          </p>
        </div>
        ...
        <div class="space-y-2">
          ...
          <p class="text-xs text-gray-500 flex items-start gap-2">
            ...
            <span
              >Este es el sonido que el paciente escuchará y deberá
              identificar</span
            >
          </p>
        </div>
        ...
        <div class="space-y-2">
          ...
          <p class="text-xs text-gray-500 flex items-start gap-2">
            ...
            <span
              >Crea opciones de respuesta y marca cuál es la correcta. Mínimo 2
              opciones.</span
            >
          </p>
        </div>
        ...
        >
          ...
          <div class="bg-white rounded-xl p-6 shadow-sm border border-blue-100">
            ...
            <div v-if="responseOptions.some((o) => o.text)">
              ...
              <div class="space-y-2">
                <label
                  v-for="(option, index) in responseOptions.filter(
                    (o) => o.text,
                  )"
                  :key="index"
                  :class="[
                ...
                >
                  <input
                    ...
                    disabled
                  />
                  <span class="font-medium text-blue-600 mr-2"
                    >{{ String.fromCharCode(65 + index) }}.</span
                  >
                  <span class="flex-1 text-gray-700">{{ option.text }}</span>
                  <span
                    v-if="option.isCorrect"
                    class="text-xs px-2 py-1 bg-green-600 text-white rounded font-semibold"
                  >
                    ✓ Correcta
                  </span>
                </label>
        ...
        <div v-if="showSuccessModal" class="app-modal-root" style="z-index: 50">
          ...
          <div class="app-modal-scrim app-modal-scrim--sheet" @click.self="showSuccessModal = false">
            ...
            <p class="text-gray-600 mb-6">
              El tamizaje auditivo ha sido guardado exitosamente.
            </p>
            <button
...
const isFormValid = computed(() => {
  ...
  const hasValidOptions =
    responseOptions.value.length >= 2 &&
...
const validateAndSetFile = (file: File) => {
  ...
  if (
    ...
    errors.value.audio =
      "Formato de archivo no válido. Use MP3, WAV, OGG o M4A";
    return;
  }
...
const handleSubmit = async () => {
  ...
  try {
    ...
    axios
      .post("http://localhost:3000/api/v1/screenings", data)
      .then((response) => {
