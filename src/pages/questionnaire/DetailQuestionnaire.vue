<template>
  <div class="container mx-auto px-4 py-8">
    <aside
      class="w-full max-w-4xl mx-auto bg-white rounded-2xl border border-indigo-100 shadow-sm overflow-hidden"
    >
      <!-- Header -->
      <div class="bg-gradient-to-r from-indigo-500 to-purple-600 px-6 py-5">
        <div class="flex items-center gap-3">
          <button
            type="button"
            class="inline-flex items-center gap-1.5 px-3 py-1.5 text-sm font-semibold text-white/90 bg-white/10 hover:bg-white/20 rounded-lg transition-colors shrink-0"
            title="Volver a la lista de cuestionarios"
            @click="router.push('/list-tinnitus-questionnaire')"
          >
            <svg
          ...
          <div class="w-px h-6 bg-white/25 shrink-0"></div>
          <div
            class="w-10 h-10 bg-white/20 backdrop-blur-sm rounded-lg flex items-center justify-center shrink-0"
          >
            <svg
      ...
      <div class="p-6">
        ...
        <div v-else-if="questionnaire" class="space-y-6">
          <!-- Información General -->
          <div
            class="bg-gradient-to-br from-indigo-50 to-purple-50 rounded-xl p-5 border border-indigo-100"
          >
            <div class="space-y-3">
              <div>
                <p
                  class="text-xs font-semibold text-indigo-600 uppercase tracking-wider mb-1"
                >
                  Nombre
                </p>
              ...
              <div class="flex items-center gap-2 pt-2">
                <div
                  class="px-3 py-1 bg-indigo-500 text-white rounded-lg text-xs font-semibold"
                >
                  {{ questionnaire.questions?.length }} pregunta{{
          ...
          <div class="space-y-4">
            ...
            >
              ...
              <div class="ml-11 space-y-2">
                ...
                >
                  <div
                    class="w-6 h-6 bg-white border-2 border-indigo-300 rounded-full flex items-center justify-center flex-shrink-0"
                  >
...
const fetchQuestionnaire = async (id: string) => {
  ...
  try {
    const response = await axios.get(
      `http://localhost:3000/api/v1/questionnaires/${id}`,
    );
