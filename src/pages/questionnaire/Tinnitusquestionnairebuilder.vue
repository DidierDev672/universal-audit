    ...
    <form @submit.prevent="handleSubmit" class="space-y-6">
      ...
      <div class="bg-white rounded-3xl shadow-xl border border-purple-100 p-8">
        ...
        <div class="space-y-6">
          ...
          <div>
            ...
            <input v-model="questionnaire.title" type="text" required
          ...
          <div>
            ...
            <textarea v-model="questionnaire.description" rows="3" required
              placeholder="Describe el propósito y alcance de este cuestionario..."
              class="w-full px-4 py-3 border-2 border-purple-200 rounded-xl focus:outline-none focus:ring-4 focus:ring-purple-100 focus:border-purple-500 transition-all resize-none"></textarea>
          </div>
        </div>
      </div>
        ...
        <div class="space-y-6">
            ...
            <div v-for="(question, index) in questions" :key="question.id"
              ...
              <div class="flex items-start justify-between mb-4">
                <div class="flex items-center gap-3">
                  <div
                    class="w-10 h-10 bg-gradient-to-br from-purple-500 to-pink-500 rounded-xl flex items-center justify-center text-white font-bold shadow-lg">
                    {{ index + 1 }}
                  </div>
                ...
                <div class="flex items-center gap-2">
                  <button v-if="index > 0" type="button" @click="moveQuestion(index, 'up')"
                    class="p-2 text-gray-600 hover:bg-white rounded-lg transition-all" title="Mover arriba">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 15l7-7 7 7" />
                    </svg>
                  </button>
                  <button v-if="index < questions.length - 1" type="button" @click="moveQuestion(index, 'down')"
                    class="p-2 text-gray-600 hover:bg-white rounded-lg transition-all" title="Mover abajo">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <div class="mb-4">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-3">
                  <label v-for="type in questionTypes" :key="type.value" :class="[
                    'flex items-center gap-3 p-4 bg-white border-2 rounded-xl cursor-pointer transition-all',
                    question.type === type.value
                      ? 'border-purple-500 bg-purple-50'
                      : 'border-gray-200 hover:border-purple-300'
                  ]">
                    <input v-model="question.type" type="radio" :value="type.value" class="w-4 h-4 text-purple-600"
              ...
              <div v-if="question.type !== 'open'" class="mb-4">
                ...
                <div class="space-y-2">
                  <div v-for="(option, optIndex) in question.options" :key="optIndex" class="flex items-center gap-2">
                    <span
                      class="w-8 h-8 bg-purple-100 rounded-lg flex items-center justify-center text-purple-600 font-semibold text-sm flex-shrink-0">
                      {{ question.type === 'single' ? String.fromCharCode(65 + optIndex) : optIndex + 1 }}
                    </span>
                    <input v-model="question.options[optIndex]" type="text" required placeholder="Opción de respuesta"
                    ...
                    <button v-if="question.options.length > 2" type="button" @click="removeOption(index, optIndex)"
              ...
              <div class="flex items-center justify-between p-3 bg-white rounded-xl">
                ...
                <label class="relative inline-flex items-center cursor-pointer">
                  <input v-model="question.required" type="checkbox" class="sr-only peer" />
                  <div
          ...
          <div class="pt-4 border-t border-purple-100">
            <button type="button" @click="addQuestion"
              class="w-full py-3 bg-gradient-to-r from-purple-600 to-pink-600 text-white rounded-xl font-medium hover:shadow-lg transition-all flex items-center justify-center gap-2">
              <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4" />
              </svg>
              {{ questions.length === 0 ? 'Agregar Primera Pregunta' : 'Agregar Pregunta' }}
            </button>
          </div>
        </div>
      </div>
      ...
      <div v-if="questions.length > 0" class="bg-white rounded-3xl shadow-xl border border-purple-100 p-8">
        ...
        <div class="bg-gradient-to-br from-purple-50 to-pink-50 rounded-2xl p-6 border-2 border-purple-200">
          ...
          <div class="space-y-6">
            <div v-for="(question, index) in questions" :key="question.id" class="bg-white rounded-xl p-5">
              ...
              <div v-else-if="question.type === 'open'">
                <textarea rows="3" placeholder="El paciente escribirá su respuesta aquí..."
                  class="w-full px-4 py-3 border-2 border-purple-200 rounded-xl bg-gray-50" disabled></textarea>
              </div>
            </div>
          </div>
        </div>
