/** Fondo tipo WhatsApp: iconos clínicos / auditivos en mosaico */
...
export type IconPatternItem = {
  id: string;
  label: string;
  /** path d de un SVG 24x24 stroke */
  path: string;
  paths?: string[];
  bg: (typeof ICON_BG_COLORS)[number];
  text: (typeof ICON_TEXT_COLORS)[number];
  rotate: number;
  /** Posición % en el wallpaper (estilo WhatsApp) */
  left?: number;
  top?: number;
  scale?: number;
  skewX?: number;
...
/** PRNG determinista a partir de semilla (cambia en cada visita / recarga) */
...
export const AI_ANALYSIS_ICON_PATTERN: IconPatternItem[] = [
  {
    id: "book",
    label: "Libro",
...
/**
 * Repite cada icono ×repeat (p. ej. ×6 = 180 iconos) y los dispersa por toda el área.
 * `layoutSeed`: distinto en cada montaje → posiciones nuevas al recargar o cambiar de ruta.
 */
/** Mezcla Fisher–Yates con PRNG determinista */
...
/**
 * Repite cada icono ×repeat (×3 = 90 iconos) y los dispersa por toda el área
 * con rejilla + jitter fuerte (mosaico tipo WhatsApp).
 */
export function buildIconWallpaper(
  ...
  for (let r = 0; r < repeat; r++) {
    AI_ANALYSIS_ICON_PATTERN.forEach((icon, i) => {
      base.push({
        ...icon,
        id: `${icon.id}-${r}-${i}`,
      });
  ...
  const cols = Math.ceil(Math.sqrt(total * 1.15));
  ...
  shuffled.forEach((icon, idx) => {
    ...
    const jitterX = (rnd() - 0.5) * cellW * 0.85;
    const jitterY = (rnd() - 0.5) * cellH * 0.85;
    const left = (col + 0.5) * cellW + jitterX;
    const top = (row + 0.5) * cellH + jitterY;
    const scale = 0.52 + rnd() * 0.72;
    const rotate = icon.rotate + (rnd() - 0.5) * 88;
    const skewX = (rnd() - 0.5) * 28;
    const skewY = (rnd() - 0.5) * 22;
    const opacity = 0.38 + rnd() * 0.42;

    out.push({
      ...icon,
      id: `${icon.id}-${idx}-${layoutSeed}`,
      left: Math.min(99, Math.max(0.5, left)),
      top: Math.min(99, Math.max(0.5, top)),
      scale,
      rotate,
      skewX,
      skewY,
